...
public class DataFinder {
	...
	public enum ConditionTypes {
		Equals, GreaterThen, LessThen, GreaterOrEqual, LessOrEqual;
	}
	...
	private class ConditionItem {
		...
		public ConditionItem(String field, ConditionTypes condition,
				Object value) {
			this.field = field;
	...
	public Persistent Next() throws InstantiationException,
	...
	void addCondition(String field, ConditionTypes condition, Object value)
		...
		conditions.add(new ConditionItem(annotation.IndexName(), condition,
				DataWorker.ConvertValueForIndexing(value)));
	}

	public DataFinder Where(String field, Object value)
			throws NoSuchFieldException, SecurityException {
		return Where(field, ConditionTypes.Equals, value);
	}

	public DataFinder Where(String field, ConditionTypes condition, Object value)
			throws NoSuchFieldException, SecurityException {
	...
	private ArrayList<Long> generateFullIds() {
		...
		NodeReference node = DataWorker.GetNodeReference(DataWorker
				.GetDataGlobalName(searchClass));
		Long key = (long) 0;
		while (true) {
	...
	private ArrayList<Long> generateIdsForCondition(ConditionItem conditionItem) {
		...
		case Equals:
			return generateIdsForEquals(node, conditionItem.field,
					conditionItem.value);
		case LessOrEqual:
		case LessThen:
		case GreaterThen:
		case GreaterOrEqual:
			return generateIdsForCompare(node, conditionItem.field,
					conditionItem.value, conditionItem.condition);
		default:
			return new ArrayList<Long>();
		}
	}

	private ArrayList<Long> generateIdsForEquals(NodeReference node,
	...
	private ArrayList<Long> generateIdsForCompare(NodeReference node,
			String indexName, Object value, ConditionTypes condition) {
		ArrayList<Long> results = new ArrayList<Long>();
		...
		while (true) {
			...
			if (condition == ConditionTypes.LessThen
					|| condition == ConditionTypes.LessOrEqual)
				fieldKey = node.previousSubscript(indexName, fieldKey);
			else
				fieldKey = node.nextSubscript(indexName, fieldKey);
		}
