...
public class DataWorker {
	...
	private void UpdateIndexForField(Persistent obj, Field field,
			Persistent oldObj) throws IllegalAccessException {
		if (field.getName() == "Id")
			return;
	...
	public void UpdateIndicesOnDelete(Persistent obj)
		...
		for (int i = 0; i < fields.length; i++) {
			...
			if (annotation != null)
				node.kill(annotation.IndexName(), ConvertValueForIndexing(field
						.get(obj).toString()), obj.Id);
		}
	}
	...
	public void DeleteObjectById(Long id, String globalName) {
		NodeReference node = ConnectionManager.Instance().getConnection()
				.createNodeReference(globalName);
		node.kill(id);
	}

	public void DeleteObject(Persistent obj) throws IllegalArgumentException,
	...
	public Persistent LoadObjectById(Long id, Persistent obj)
			throws IllegalAccessException {
		NodeReference node = connection.createNodeReference(obj
				.GetStorageGlobalName());
		...
		do {
			...
			if (subscript.length() > 0) {
				...
				FieldGetter.GetFieldValue(obj, field, nodeValue, subscript,
						node);
			}
