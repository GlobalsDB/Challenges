...
public class FieldGetter {
	public static void GetFieldValue(Persistent obj, Field field,
			Object nodeValue, String subscript, NodeReference node) {
		...
		try {
			...
			if (Persistent.class.isAssignableFrom(field.getType())) {
				...
				try {
					fieldAsPersistentObject = (Persistent) field
							.getDeclaringClass().newInstance();
					fieldAsPersistentObject.Id = Long.parseLong(nodeValue
							.toString());
					fieldAsObj = DataWorker.Instance()
							.LoadObjectById(fieldAsPersistentObject.Id,
			...
			if (nodeValue instanceof java.lang.Long) {
				...
				if (field.getType() == java.util.Date.class) {
					field.set(obj,
							DateHelper.StringToDate(nodeValue.toString()));
				} else {
